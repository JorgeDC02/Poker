...
public class Jugador {
   ...
   private int puntos = 0;
    ...
    public void mostrarMano(int NumPlayer){
       System.out.println("Cartas para jugador "+NumPlayer+"\n"+this.mano);
    }
    ...
    public void prepararJugada(Carta card,int numJugador){
        if(manoJugada.size() == 0){//En caso de que no haya ninguna combinacion de cartas ganadoras
            ...
            descartarCarta(numJugador);
            cogerCarta(card, numJugador);
        }
        else {
            mostrarJugada(numJugador);
            this.calcularPuntuacion();
        }
    }
    //Metodo que descarta una carta y la retorna para añadir a baraja
    public Carta descartarCarta(int numJugador){
        int numCarta =(int)(Math.random()*4+1);//Descarta aleatoriamente una carta no repetida
        System.out.println("Jugador "+numJugador+" descartara la carta: "+manoDescartes.get(numCarta));
        cartaDescarte = manoDescartes.get(numCarta);
        manoDescartes.remove(numCarta);
        return cartaDescarte;
    }
    
    public void cogerCarta(Carta card, int numJugador){
        ...
        System.out.println("Jugador "+numJugador+" cogerá una carta\nCartas de la mano actualizadas: "+mano);
    }
    
    public void mostrarJugada(int numJugador){
        System.out.println("Combinacion de cartas ganadora del jugador "+numJugador+": "+this.manoJugada);
    }   
    ...
    //valores: pareja->*1 | doble pareja->*10 | trío->*100 | full->*1000 | poker->*10000
     public int calcularPuntuacion(){
        switch(this.manoJugada.size()){
            case 2: this.puntos = this.manoJugada.get(0).getNumero()*2*1;
            break;
            case 3: this.puntos = this.manoJugada.get(0).getNumero()*3*100;
            break;
            case 4: int cont=0;
                    ...
                    if(cont == 4){
                        this.puntos = this.recorrerManoJugada()*10000;
                    }
                    else this.puntos = this.recorrerManoJugada()*10;
            break;
            case 5: this.puntos = this.recorrerManoJugada()*1000;
            break;
        }
        return this.puntos;
    }
